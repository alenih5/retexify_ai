...
class ReTexify_AI_Pro_Light {
    ...
    public function add_admin_menu() {
        add_menu_page(
            ...
            'dashicons-superhero', // Ein Test-Icon
            25 // Position
        );
    }
    ...
    // DEUTSCHE CONTENT-ANALYSE FUNKTIONEN
    ...
    private function extract_german_keywords($text) {
    ...
    private function identify_german_business_themes($text) {
        $text = mb_strtolower($text, 'UTF-8');
        $themes = array();
    ...
    private function assess_german_content_quality($content, $word_count) {
        $score = 0;
    ...
    public function handle_get_page_content() {
        ...
        try {
            ...
            // DEUTSCHE CONTENT-ANALYSE (VERBESSERT)
            ...
            // DEUTSCHE KEYWORDS EXTRAHIEREN
            $german_keywords = $this->extract_german_keywords($content . ' ' . $post->post_title);
            
            // BUSINESS-THEMEN ERKENNEN
            $business_themes = $this->identify_german_business_themes($content . ' ' . $post->post_title);
            
            // CONTENT-QUALITÄT BEWERTEN
            $content_quality = $this->assess_german_content_quality($content, $word_count);
            
            // SCHWEIZER REGIONALE BEGRIFFE ERKENNEN
            $regional_info = $this->detect_swiss_regional_content($content . ' ' . $post->post_title);
    ...
    private function generate_single_seo_item($post, $seo_type, $settings, $include_cantons, $premium_tone) {
        ...
        // Schweizer Kantone Logik verbessert
        $canton_text = '';
        if ($include_cantons && !empty($settings['target_cantons'])) {
            ...
            if (!empty($selected_canton_names)) {
                ...
                $canton_text .= 'Berücksichtige diese Kantone für lokale SEO-Optimierung.';
            }
        }
        
        // Premium Business-Ton Logik verbessert
        $tone_instruction = '';
        if ($premium_tone) {
            $tone_instruction = 'PREMIUM BUSINESS-MODUS: Verwende einen exklusiven, hochwertigen Ton für anspruchsvolle Kunden. Betone Qualität, Expertise und Schweizer Präzision.';
        } else {
            $tone_instruction = 'STANDARD-MODUS: Verwende einen professionellen, aber zugänglichen Ton für allgemeine Schweizer Kunden.';
        }
...
- **Vermeide übertriebene Marketing-Begriffe wie 'Luxus'. Konzentriere dich auf die tatsächliche Dienstleistung.**
...
Antworte nur mit dem Keyword, nichts anderes:"
    ...
    private function generate_complete_seo_suite($post, $settings, $include_cantons, $premium_tone) {
        ...
        if ($include_cantons && !empty($settings['target_cantons'])) {
            ...
            if (!empty($selected_canton_names)) {
                $canton_text = 'Ziel-Kantone: ' . implode(', ', $selected_canton_names) . "\n";
                $canton_text .= 'Berücksichtige diese Kantone in der SEO-Optimierung für lokale Suchergebnisse.';
            }
        }
        ...
        $tone_instruction = '';
        if ($premium_tone) {
            $tone_instruction = 'PREMIUM BUSINESS-MODUS: 
...
- Zielgruppe: Entscheidungsträger und Premium-Kunden';
            ...
            $tone_instruction = 'STANDARD-MODUS: 
- Erstelle professionelle, aber zugängliche Texte
- Verwende einen freundlichen, verständlichen Ton
- Zielgruppe: Allgemeine Schweizer Kunden';
        }
...
- **Für das Focus-Keyword:** Verwende kommerzielle Begriffe mit Suchvolumen. Vermeide übertriebene Marketing-Sprache wie 'Luxus'.
...
Erstelle jetzt eine optimierte SEO-Suite:";
    ...
    private function get_light_css() {
        ...
        .retexify-main-actions {
            display: flex;
            flex-wrap: wrap;
            gap: 15px;
        }
        
        .retexify-seo-actions .retexify-btn {
            width: 100%;
            justify-content: center;
        }
        
        .retexify-seo-actions .retexify-btn-large {
            min-width: 250px;
            font-size: 16px;
            font-weight: 700;
        }
        ...
        .retexify-settings-grid > .retexify-settings-group:nth-child(1) {
            grid-column: span 1;
        }
        .retexify-settings-grid > .retexify-settings-group:nth-child(2) {
            grid-column: span 2;
        }
    ...
    private function get_admin_js() {
        ...
        jQuery(document).ready(function($) {
            ...
            $(".retexify-tab-btn").click(function() {
                console.log("Tab geklickt:", $(this).data("tab"));
            ...
            function loadDashboard() {
                console.log("loadDashboard() aufgerufen");
                ...
                $.post(retexify_ajax.ajax_url, {
                    ...
                    $("#retexify-dashboard-content").html("<div class=\"retexify-warning\">AJAX Fehler: " + error + "</div>");
                });
            }
            
            // Dashboard refresh
            $("#retexify-refresh-stats").click(function() {
                console.log("Refresh Stats geklickt");
                loadDashboard();
            });
            ...
            function displayCurrentSeoPage() {
                ...
                $("#retexify-page-url").attr("href", current.url).attr("target", "_blank");
                $("#retexify-edit-page").attr("href", current.edit_url).attr("target", "_blank");
            ...
            $("#retexify-generate-all-seo").click(function() {
                ...
                var $btn = $(this);
                var originalText = $btn.html();
                $btn.html("🇨🇭 Generiere KI-SEO-Suite...").prop("disabled", true);
                ...
                $.post(retexify_ajax.ajax_url, {
                    ...
                    if (response.success) {
                        ...
                        $("#retexify-seo-results").html("<div class=\"retexify-test-success\">✅ KI-SEO-Suite erfolgreich generiert!</div>");
                    } else {
                        alert("KI-SEO-Suite Generierung fehlgeschlagen: " + response.data);
                    }
                });
            });
            ...
            // LINK-HANDLING FUNKTION
            function setupPageLinks() {
                // Seite anzeigen Link
                $("#retexify-page-url").off("click").on("click", function(e) {
                ...
                // Bearbeiten Link
                $("#retexify-edit-page").off("click").on("click", function(e) {
                    var url = $(this).attr("href");
                    if (url && url !== "#") {
                        window.open(url, "_blank");
                    } else {
                        e.preventDefault();
                        alert("Bearbeitungs-URL nicht verfügbar");
                    }
                });
            }
...
// End of ReTexify AI Pro Light Plugin
